...
$execute unless score $(cityName) cityOwnership matches 1 if score $(cityName) cityConquerProgress >= $(cityName) cityConquerValue as @s[team=Druids] run function mythcraft:conquer {cityName:$(cityName), teamId:1, teamName:Druids}
$execute unless score $(cityName) cityOwnership matches 2 if score $(cityName) cityConquerProgress >= $(cityName) cityConquerValue as @s[team=Royals] run function mythcraft:conquer {cityName:$(cityName), teamId:2, teamName:Royals}
