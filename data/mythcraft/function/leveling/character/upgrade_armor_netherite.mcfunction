...
# Warrior (1) and Bastion (3) have diamond head+chest → netherite
$execute unless score @s playerClass matches 2 run item replace entity @s armor.head with netherite_helmet[attribute_modifiers=[],unbreakable={},enchantments={"minecraft:vanishing_curse":1},enchantment_glint_override=false,trim={material:$(trimMaterial),pattern:$(trimPatternHead)}] 1
