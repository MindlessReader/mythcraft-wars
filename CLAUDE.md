...
The tick loop (`tick.mcfunction`) handles: player rekit on death, spell cooldown ticking, troop indicator (XP bar), troop activation/deactivation by player proximity, evoker vex team-tagging, slime troop split cleanup, buff reapplication, and trigger detection for the player menu and teleport systems.
...
- Bastion parry: `parryWindow` (ticks remaining in perfect parry window), `parryCooldown` (ticks remaining on cooldown), `parryShieldProtect` (ticks remaining of shield disable prevention after perfect parry; 100 = 5s). Temp: `_shieldTick` (blocking this tick), `_wasBlocking` (was blocking last tick)
...
- **Troop spawn templates:** 11 regular (pillager, vindicator, zombie, skeleton, witch, husk, parched, stray, slime, bogged, piglin) + 10 boss (bosszombie, bossskeleton, bosswitch, blaze, breeze, illusioner, witherskeleton, evoker, piglinbrute, bossslime) in `respawn/spawn/*.mcfunction`. Special mob handling: piglins/piglin brutes use `IsImmuneToZombification:1b` to prevent overworld conversion; slimes use `Size:1` for attack AI (children inherit tags, cleaned up per-tick in `tick.mcfunction`); evoker vexes are team-tagged each tick via `tick.mcfunction` to prevent friendly fire; parched/husk are sun-immune (`burning_time:-1`); stray/bogged burn in daylight (use iron helmet + `burning_time:2`)
