...
- Raid boss: `raidBossState` (fake player `RaidBoss`: 0=not spawned, 1=alive, 2=defeated), `raidBossHP`/`raidBossMaxHP` (scoreboard-authoritative HP synced from entity on each hit), `raidBossLastHit` (1=Team1, 2=Team2). Math constants: `C_2`, `C_3`, `C_4` on `mathCounter` (for HP threshold calculation)
...
- **Raid boss:** One-per-game world boss. `raidboss/schedule_spawn` picks random quest slot [0, questCount-1] (0=pre-game); `check_spawn` (called from `startquest`) compares current quest to slot and schedules `try_spawn` with random delay within [15, duration-15]s. `try_spawn` checks `recentTransition` flag (set in startquest/endquest, cleared 15s later) to avoid quest boundary collisions — reschedules 15s if flagged. Builds eligible location list from `raidBossPool` config + existing `spawnmarker_raidboss` markers. Boss entity tagged `raidBoss` (NOT `cityTroop`), on Unaligned team. Advancement-driven: `player_hurt_entity` → `on_hit` (team recording + HP sync + threshold checks at 75/50/25%), `player_killed_entity` → `on_kill` → `on_death` (VP + all rewards + cleanup). 1s scheduled `monitor` loop for tether (>30 blocks from marker → TP back) + fallback death detection. Slime split children killed via `cleanup_splits` 2 ticks after death. Config: per-location `raidBossPool` (boss type IDs), global `rewards.raidBossPool` (ALL enabled granted, not random). Boss display names in `mythcraft:raidbossnames` storage. Player menu shows status via pre-built `raidBossLine1`/`raidBossLine2` strings (helper functions for string concatenation).
